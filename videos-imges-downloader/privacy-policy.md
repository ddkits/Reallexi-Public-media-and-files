...
- Saved media blobs selected by the user.
- Media metadata such as title, filename, MIME type, size, source URL, source page URL, dimensions, duration, and save date.
...
Data is used only to provide user-requested extension features:
...
Saved files, metadata, settings, and task status remain in the user's browser profile using extension-owned storage such as IndexedDB and `chrome.storage.local`.
...
Users can clear saved media from the extension library. Users can also remove all extension data by uninstalling the extension or clearing extension storage in the browser.
